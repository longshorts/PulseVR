Shader "Transparent/Reflex Sight (Rotated, Reflective)" {
	Properties {
		...
		_reflectColor ("Reflection Colour", Color) = (1,1,1,1)
	...
	SubShader {
		...
		float4 _reticleColour, _glassColour, _reflectColor;
		samplerCUBE _cube;
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			float3 env = texCUBE(_cube, IN.worldRefl);
   			float reflectivity = dot(_reflectColor.rgb, env.rgb);
			
			o.Emission = col.a * _reticleColour.rgb * _reticleBright + (1 - col.a) * env.rgb * reflectivity * _reflectColor;
